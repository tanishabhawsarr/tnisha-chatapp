...
import { LETSTALK_TOKEN } from "../constants/config.js";
import { User } from "../models/user.js";
 
const isAuthenticated=TryCatch((req,res,next)=>{

    const token=req.cookies[LETSTALK_TOKEN];
...
const adminOnly=(req,res,next)=>{
    ...
    const token=req.cookies["Lestalk-admin-token"];
...
const socketAuthenticator=async(err,socket,next)=>{
    try {
        ...
        const authToken=socket.request.cookies[LETSTALK_TOKEN];
